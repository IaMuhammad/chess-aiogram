/* screens.jsx — Home, Matchmaking, Lobby, ResultSheet. Presentational; logic in app. */
...
// decorative mini board behind the hero
...
// ── HOME ─────────────────────────────────────────────────────────
function HomeScreen({ me, tc, setTc, onPlayFriend, onFindOpponent, onWatch, recent }) {
  return (
    <div className="tg-app">
      <TgHeader
        ...
        accentSub
        onMenu={() => {}}
        right={<button className="tg-hbtn" onClick={onWatch} aria-label="Spectate" style={{ color: 'var(--sub)' }}><Icon name="eye" size={21} /></button>}
      />
      <div className="tg-scroll">
        {/* hero / profile */}
        <div style={{ position: 'relative', margin: '12px 12px 0', borderRadius: 18, overflow: 'hidden',
          background: 'linear-gradient(135deg, color-mix(in srgb, var(--accent) 32%, var(--panel)), var(--panel))', padding: 16 }}>
          <MiniBoardBanner />
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 13 }}>
            <Avatar name={me.name} size={54} color={me.color} online />
              ...
              {recent.map((g, i) => (
                <div key={i}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 13px' }}>
                    <Avatar name={g.name} size={36} color={g.color} />
                    <div style={{ flex: 1, minWidth: 0 }}>
...
// ── MATCHMAKING ──────────────────────────────────────────────────
function MatchmakingScreen({ tc, elapsed, found, opponent, onCancel }) {
  return (
    <div className="tg-app">
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, gap: 22 }}>
        <div style={{ position: 'relative', width: 150, height: 150, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'relative', transition: 'transform .4s' }}>
            {found
              ? <Avatar name={opponent.name} size={92} color={opponent.color} online />
...
// ── LOBBY (play a friend) ────────────────────────────────────────
function LobbyScreen({ tc, link, joined, opponent, me, onCopy, onShare, onStart, onBack, toast }) {
  return (
    <div className="tg-app">
      ...
      <div className="tg-scroll" style={{ padding: '14px 12px' }}>
        ...
        <div className="card" style={{ padding: '18px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-around', marginBottom: 14 }}>
          <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7 }}>
            <Avatar name={me.name} size={56} color={me.color} online />
          ...
          <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7 }}>
            {joined
              ? <Avatar name={opponent.name} size={56} color={opponent.color} online />
        ...
        <div className="card" style={{ padding: 12, display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0, fontSize: 13.5, color: 'var(--sub)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{link}</div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button className="btn btn-ghost" style={{ flex: 1 }} onClick={onCopy}><Icon name="qr" size={18} /> QR code</button>
        </div>

        <div style={{ marginTop: 16, fontSize: 12.5, color: 'var(--hint)', textAlign: 'center', lineHeight: 1.5, padding: '0 14px' }}>
          {joined ? 'Your friend is ready. Start when you are!' : 'Share this link in any chat. The game begins as soon as they open it.'}
        </div>
      </div>

      <div className="controls" style={{ borderTop: 'none' }}>
        <button className="btn btn-primary btn-block" disabled={!joined} onClick={onStart}>
          {joined ? 'Start game' : 'Waiting for opponent…'}
        </button>
      </div>
...
// ── RESULT SHEET ─────────────────────────────────────────────────
function ResultSheet({ result, title, reason, delta, newRating, onRematch, onHome, onReview }) {
    ...
    <div className="overlay">
      <div className="sheet">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          <button className="btn btn-primary btn-block" onClick={onRematch}><Icon name="redo" size={19} /> Rematch</button>
...
Object.assign(window, { TIME_CONTROLS, HomeScreen, MatchmakingScreen, LobbyScreen, ResultSheet });

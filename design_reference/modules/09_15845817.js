/* game.jsx — game-screen layout: player rows, moves list, chat, controls, review nav.
   Stateless/presentational; all logic lives in app.jsx. */

function PlayerRow({ player, clockMs, active, captured, adv, lowAt }) {
  const low = clockMs <= (lowAt || 20000);
  return (
    <div className="player-row">
      <Avatar name={player.name} size={40} color={player.color} online={player.online} />
...
function ChatPanel({ messages, draft, setDraft, onSend, scrollRef }) {
  return (
    <>
      <div className="panel-body" ref={scrollRef}>
        <div className="chat">
...
function GameScreen(props) {
  const {
    ...
    messages, draft, setDraft, onSend, chatRef, unread,
    isLive, atStart, atEnd, onStep, onLive,
    role, playing, onFlip, onResign, onDraw, drawBanner, onAcceptDraw, onDeclineDraw,
    statusNote,
  } = props;
  ...
  return (
    <div className="tg-app">
      ...
      <PlayerRow player={topPlayer} clockMs={topClock} active={activeSide === topPlayer.side} captured={topCaptured} adv={topAdv} lowAt={lowAt} />
      {board}
      <PlayerRow player={bottomPlayer} clockMs={bottomClock} active={activeSide === bottomPlayer.side} captured={bottomCaptured} adv={bottomAdv} lowAt={lowAt} />
      ...
      {drawBanner && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'color-mix(in srgb, var(--accent) 16%, var(--bg))', borderTop: '1px solid var(--border)' }}>
          ...
          <span style={{ flex: 1, fontSize: 13.5 }}>{topPlayer.name} offers a draw</span>
      ...
      <div className="panel">
        ...
        {tab === 'moves'
          ...
          : <ChatPanel messages={messages} draft={draft} setDraft={setDraft} onSend={onSend} scrollRef={chatRef} />}
      </div>
      ...
      {tab === 'moves' && (
        <div className="controls">
          ...
          {role === 'player' && playing && <button className="ctrl" onClick={onDraw}><Icon name="draw" size={20} /> Draw</button>}
          {role === 'player' && playing && <button className="ctrl" data-danger="true" onClick={onResign}><Icon name="flag" size={20} /> Resign</button>}
          {(role === 'spectator' || !playing) && <button className="ctrl" onClick={onExit}><Icon name="home" size={20} /> Exit</button>}
        </div>
      )}
    </div>
  );
...
Object.assign(window, { PlayerRow, MovesList, ChatPanel, GameScreen });

// Shared building blocks: piece glyphs, icons, avatars, header, clock.
// Ported from the original design (window globals → ES module exports).
import React from 'react'

export const SOLID = { k: '♚', q: '♛', r: '♜', b: '♝', n: '♞', p: '♟' }
export const PIECE_VALUE = { p: 1, n: 3, b: 3, r: 5, q: 9, k: 0 }

// Two-tone piece rendering (style: 'flat' | 'd3').
export function pieceCSS(piece, style) {
  const isWhite = piece[0] === 'w'
  const base = { fontFamily: 'var(--piece-glyph)', WebkitFontSmoothing: 'antialiased' }
  if (isWhite) {
    base.color = '#f4f7fb'
...
export function Piece({ piece, style, className, extraStyle }) {
...
export function Icon({ name, size = 22, color = 'currentColor', stroke = 2, fill = 'none', style }) {
  return (
...
const GRAD_KEYS = Object.keys(AV_GRADIENTS)

// stable gradient from a name so each player has a consistent colour
function gradientFor(name, color) {
  if (color && AV_GRADIENTS[color]) return AV_GRADIENTS[color]
  let h = 0
  for (const ch of name || '?') h = (h * 31 + ch.charCodeAt(0)) >>> 0
  return AV_GRADIENTS[GRAD_KEYS[h % GRAD_KEYS.length]]
}

export function Avatar({ name, size = 40, color, online, photoUrl }) {
  const g = gradientFor(name, color)
  const initials = (name || '?').split(' ').map((w) => w[0]).slice(0, 2).join('').toUpperCase()
  const style = photoUrl
    ? { width: size, height: size, backgroundImage: `url(${photoUrl})` }
    : { width: size, height: size, fontSize: size * 0.4, background: `linear-gradient(145deg, ${g[0]}, ${g[1]})` }
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <div className="avatar" style={style}>{photoUrl ? '' : initials}</div>
      {online && (
        <span style={{
...
// ── Telegram header ──────────────────────────────────────────────
export function TgHeader({ title, subtitle, onBack, onMenu, right, accentSub }) {
  return (
...
// captured material from a board → { capByWhite, capByBlack, adv }
